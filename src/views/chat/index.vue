        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="16" :xl="16">
          ...
          <div class="KGQA">
            <el-input
              ...
              @keyup.enter.native="handleChatData"
            >
              <template #append>
                <el-button :icon="Search" @click="handleChatData" />
              </template>
            </el-input>
          </div>
            ...
            <el-scrollbar height="600px">
              <div class="QuestionAnser" v-for="item in questionList" :key="item.id">
                ...
                <div class="answer">
                  ...
                  ></el-avatar> -->
                  <el-image
                    style="width: 40px; height: 40px; margin-left: 10px; flex-shrink: 0"
                    :src="picture"
                  />
                </div>
              </div>
...
<script setup>
  ...
  const introduce = '双碳大模型基于ChatGLM2-6B-32k大模型提供对双碳问题的智能分析与回复，具体可参照下方事例。\n';
  ...
  const handleChatData = async () => {
    const { data } = await Chatresponse({ key: ChatKey.value });
    questionList.value.push({
      id: questionList.value.length,
      question: ChatKey.value,
      answer: data,
    });
  };
  const KGQA = reactive({});

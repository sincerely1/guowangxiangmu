        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="16" :xl="16">
          ...
          <div class="KGQA">
            <el-input
              ...
              @keyup.enter.native="handleGenData"
            >
              <template #append>
                <el-button :icon="Search" @click="handleGenData" />
              </template>
            </el-input>
          </div>
...
<script setup>
  ...
  import { Genresponse } from '@/api/generation';
  ...
  const handleGenData = async () => {
    const { data } = await Genresponse({ key: GenKey.value });
    questionList.value.push({
      id: questionList.value.length,
      question: GenKey.value,
      answer: data,
    });
  };
</script>
